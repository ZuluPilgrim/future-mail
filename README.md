...
> **Note:** Docker is the recommended deployment method. The local install option is intended for development only. HTTPS support is documented below but Docker handles this more cleanly.

### Prerequisites

- **Node.js 18+** — [nodejs.org](https://nodejs.org). Node 22 LTS recommended.
- **openssl** — required for HTTPS certificate generation.
  - **Linux / macOS:** installed by default on most systems. Verify with `openssl version`.
  - **Windows:** not installed by default. Download from [slproweb.com/products/Win32OpenSSL.html](https://slproweb.com/products/Win32OpenSSL.html) and ensure it is on your PATH. Alternatively set `HTTPS_ENABLED=false` in your `.env` file to run over plain HTTP.
- An **SMTP account** for sending mail — configured in-app after setup.

### HTTPS (local)

HTTPS is enabled by default (`HTTPS_ENABLED=true`). On first run the server will auto-generate a self-signed certificate at `./data/certs/cert.pem` using `openssl`. Your browser will show a certificate warning — click through it or import the cert into your trust store to remove it permanently.

To disable HTTPS locally, add this to a `.env` file in the project root:
```
HTTPS_ENABLED=false
```
